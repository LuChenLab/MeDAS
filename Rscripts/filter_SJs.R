...
multi_merge <-
  function(path,
    ...
    tab_list <-
      mclapply(
        ...
        function(f) {
          ...
          colnames(tab)[2] <- sub("[^0-9a-zA-Z]{0,}SJ.out.tab$", "", f)
    ...
    merged_tab <-
      Reduce(
        function(x, y) {merge(x, y, by='junctions', all = T)},
        tab_list
      )
    
    info <- data.frame(stringr::str_match(merged_tab$junctions, "(.*):(.*):(.*):(.*):(.*):(.*)"), stringsAsFactors=F)
    info <- info[, 2:7]
    ...
    SJ_2_keep <- 
      merged_tab[, .(Total_cnt = apply(.SD, 1, sum, na.rm = TRUE), Total_sample = apply(.SD, 1, function(x) {sum(sum(x >= minSJ, na.rm = TRUE))}))]
    
    info <- info[SJ_2_keep[, Total_cnt >= minSJs & Total_sample >= minSamps], ]
